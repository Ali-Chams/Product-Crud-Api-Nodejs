...
//     description:
//       "How could someone resist these lovely earrings? Right - they cant.",
//     price: 129.59,
...
//     _id: "lkljlkk11",
//     name: "A T-Shirt",
//     description: "This T-Shirt can soon be yours. If you find that buy button.",
//     price: 39.99,
...
router.get("/:id", (req, res, next) => {
  db.getDb().db()
  .collection("products")
  .findOne({_id:new ObjectId(req.params.id)}).then(productDoc => {  
      if (!productDoc) {
...
router.patch("/:id", (req, res, next) => {
  ...
  db.getDb().db().collection("products").updateOne({_id: new ObjectId(req.params.id)},{$set: updatedProduct})
  .then( result =>{
...
router.delete("/:id", (req, res, next) => {
  db.getDb().db().collection('products').deleteOne({_id: new ObjectId(req.params.id)})
  .then(result =>{ res.status(200).json({message:'Product Deleted'})}).catch(err=>{
    console.log(err);
    res.status(500).json({ message: "An error occured"});
  });
});
