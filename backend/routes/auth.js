...
router.post('/login', (req, res, next) => {
 ...
 db.getDb().db().collection('users').findOne({ email: email })
    ...
    res
    .status(200)
    .json({ message: 'Authentication failed, invalid username or password.',token:token });
    })
    .catch(err=>{
     res
    .status(401)
    .json({ message: 'Authentication succeded' });
    })
...
router.post('/signup', (req, res, next) => {
  ...
  bcrypt
    .hash(pw, 12)
    .then(hashedPW => {
      db.getDb().db().collection('users').insertOne({email:email,
        password:hashedPW
      }).then(result =>{
        console.log(result);
         const token = createToken();
